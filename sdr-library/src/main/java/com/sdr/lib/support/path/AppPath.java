...
public class AppPath {
    ...
    private final static String APP_CACHE = SDR_LIBRARY.getInstance().getApplication().getFilesDir().getAbsolutePath();

    // 存储用户信息的地址
    private final static String USER_INFO_CACHE = APP_CACHE + File.separator + "user";

    // sd卡中应用的缓存地址
    private final static String APP_SD_CACHE = SDR_LIBRARY.getInstance().getApplication().getExternalCacheDir().getAbsolutePath();
