...
public class AppPath {
    ...
    private final static String APP_CACHE = MyApplication.getApplication().getFilesDir().getAbsolutePath();

    // 存储用户信息的地址
    private final static String USER_INFO_CACHE = APP_CACHE + File.separator + "user";

    // sd卡中应用的缓存地址
    private final static String APP_SD_CACHE = MyApplication.getApplication().getExternalCacheDir().getAbsolutePath();
    ...
    // ------------------------------------------------------------------------------
    // sd 卡中的路径
    private final static String APP_PATH = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + MyApplication.getApplication().getPackageName();
